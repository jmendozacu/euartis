...
class VladimirPopov_WebForms_Block_Webforms extends Mage_Core_Block_Template
{
	...
	protected function _prepareLayout()
	{
		...
		//proccess texts
		if((float)substr(Mage::getVersion(),0,3)>1.3){
			$webform->setDescription(
				Mage::helper('cms')->getPageTemplateProcessor()->filter($webform->getDescription())
			);
			$webform->setSuccessText(
				Mage::helper('cms')->getPageTemplateProcessor()->filter($webform->getSuccessText())
			);
		}
