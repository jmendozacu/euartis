 ...
 */
class Magestore_Showroom_Block_Catalog_Product_Abstract extends Mage_Catalog_Block_Product_List
{
    ...
    public function getViewKey()
    {
        if ($skey = $this->getRequest()->getParam('skey')){
            return $skey;
        } elseif ($skey = Mage::registry('magestore_showroom_session_key')) {
            return $skey;
        }        
        Mage::register('magestore_showroom_session_key', hash('ripemd160', time()));
        return Mage::registry('magestore_showroom_session_key');
    }
